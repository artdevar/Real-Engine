...
public:
  CStopwatch() : m_Message(),
                 m_Start(std::chrono::high_resolution_clock::now())
  {
  }

  CStopwatch(std::string_view _Message) : m_Message(_Message.data()),
                                          m_Start(std::chrono::high_resolution_clock::now())
  {
  }
  ...
  ~CStopwatch()
  {
    ...
    const std::string LogMessage = m_Message.has_value() ? std::format("[{}] Time execution: {}", m_Message.value(), Duration) : std::format("Time execution: {}", Duration);
