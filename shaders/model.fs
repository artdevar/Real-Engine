...
vec3 CalcDirLight(TLightDirectional _Light, vec3 _Normal, vec3 _ViewDir)
{
  ...
  vec3 ambient  = _Light.Ambient * texture(u_Material.DiffuseTexture, io_TexCoords).rgb;
  vec3 diffuse  = _Light.Diffuse * diff * texture(u_Material.DiffuseTexture, io_TexCoords).rgb;
  vec3 specular = _Light.Specular * spec * texture(u_Material.SpecularTexture, io_TexCoords).rgb;
